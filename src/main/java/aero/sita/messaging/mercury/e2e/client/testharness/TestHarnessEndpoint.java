...
public enum TestHarnessEndpoint {
  ...
  /**
   * Endpoint for sending messages to IBM MQ via test-harness.
   * POST /api/v1/ibm/send
   */
  SEND_MESSAGE("/api/v1/ibm/send"),

  /**
   * Endpoint for retrieving all received messages.
   * GET /api/v1/received
   */
  GET_RECEIVED("/api/v1/received"),

  /**
   * Endpoint for retrieving latency information.
   * POST /api/v1/results/latency
   */
  GET_LATENCY("/api/v1/results/latency"),

  /**
   * Endpoint for clearing all delivered messages.
   * POST /api/v1/results/clear
   */
  CLEAR_MESSAGES("/api/v1/results/clear"),

  /**
   * Endpoint for retrieving result by ID.
   * GET /api/v1/results/{id}
   */
  GET_RESULT_BY_ID("/api/v1/results/{id}");
   ...
   */
  TestHarnessEndpoint(String path) {
    this.path = path;
  }
  ...
  public String buildUrl(String baseUrl, Object... params) {
    ...
    if (this == GET_RESULT_BY_ID) {
      if (params.length == 0) {
        throw new IllegalArgumentException("GET_RESULT_BY_ID requires an ID parameter");
      }
